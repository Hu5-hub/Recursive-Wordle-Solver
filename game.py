...
class WordleEvaluation:
    ...
    def play(self):
        guess_total = 0
        play_time = 0
        for _ in tqdm(range(len(self.player.pool))):
            self.round += 1
        ...
        print(f"Average number of guesses: {avg_num_guesses:.2f}")
        print(f"Play rate (games per sec): {play_rate:.2f}")
...
def main():
    ...
    parser.add_argument('-m', '--mode', dest='mode', required=False, default="interactive",
                        help='game mode: interactive, continuous, histogram, evaluation')
